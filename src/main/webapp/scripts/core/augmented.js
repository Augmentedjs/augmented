...
(function(root, factory) {
	...
	var Validator = function() {
		...
		var tv4 = createApi();
		tv4.addLanguage('en-us', ErrorMessagesDefault);  // changed to US Engilsh
	...
	var i18nBase = function() {
		 ...
		 * Eg, prop('com.company.bundles.menu_add')
		 */
		this.prop = function(key /* Add parameters as function arguments as necessary  */) {
	...
	Augmented.Utility.MessageReader = {
		getMessage: function(key) {
			...
			var msg = Augmented.Utility.ResourceBundle.getString.apply(this,arguments),
				...
				last = key.length,
				originalKey = key;
			//if message is not found, then ResourceBundle returns the key wrapped in square brackets
			//loop through the fallback path of the key by removing the last attribute and searching the bundle again
			...
			//if the original key or a fallback was found, return the message
			//otherwise return the original key with square brackets (default jquery.i18n.properties plugin result)
	 ...
	 * 
	 * concatenate the pieces of the error together
	 * if a portion of the key is missing, the rest of the key is ignored.
	 * ex. if the "rule" attribute is missing, then the key will return with
	 *     the error.level + error.kind only
	 */
	Augmented.Utility.MessageKeyFormatter = {
			delimiter: ".",
	...
	/**
	 * Abstract Augmented Model Supports: 
	 * CORS 
	 * Schemas 
	 * Security * TODO: implement OAUTH 2
	 */
